...
with open(Path(__file__).parent / "2022_11_sample.txt") as fp:
    RAW_SAMPLE = fp.read()


with open(Path(__file__).parent / "2022_11_input.txt") as fp:
    RAW_INPUT = fp.read()


class Monkey:
    def __init__(self, raw_observation) -> None:
        self.inspections = 0
        for obs_line in raw_observation.split("\n"):
            if obs_line[0] != " ":
                continue
            tag, val = obs_line.split(": ")
            if tag == "  Starting items":
                self.items = [int(v) for v in val.split(", ")]
                continue
            vals = val.split(" ")
            last_val = int(vals[-1]) if vals[-1].isdigit() else None
            if tag == "  Operation":
                # new = old */+ old/n
                self.op_string = vals
                op_factor = last_val  # weird scope thing without lambda took last_val
                self.op_factor = op_factor
                if last_val is not None:
                    if vals[-2] == "*":
                        self.op = lambda x: (x * op_factor)
                    elif vals[-2] == "+":
                        self.op = lambda x: (x + op_factor)
                else:
            ...
            if tag == "  Test":
                # divisible by n
                self.test_div_by = last_val
                continue
            if tag == "    If true":
                # throw to monkey n
                self.true_target = last_val
                continue
            if tag == "    If false":
                # throw to monkey n
                self.false_target = last_val
                continue

    def take_turn(self, calm=True, mod_worry=None):
        for_true = []
        for_false = []
        for item in self.items:
            ...
            if mod_worry is not None:
                new_level = new_level % mod_worry
            if new_level % self.test_div_by == 0:
                for_true.append(new_level)
            else:
                for_false.append(new_level)
        self.items = []
        return self.true_target, for_true, self.false_target, for_false


class MonkeyGames:
    def __init__(self, raw_observations, calm=True) -> None:
        self.monkeys = []
        self.calm = calm
        self.mod_worry = None
        for raw_observation in raw_observations.split("\n\n"):
            self.monkeys.append(Monkey(raw_observation))
        if not calm:
            self.mod_worry = 1
            for monkey in self.monkeys:
                self.mod_worry *= monkey.test_div_by

    def play_round(self):
        for monkey in self.monkeys:

            true_target, for_true, false_target, for_false = monkey.take_turn(
                calm=self.calm, mod_worry=self.mod_worry
            )
            self.monkeys[true_target].items += for_true
            self.monkeys[false_target].items += for_false

    def get_monkey_business(self):
        max_inspections = sorted([m.inspections for m in self.monkeys])[-2:]
        return max_inspections[0] * max_inspections[1]


def test_monkeys():
    # logging.basicConfig(level=logging.DEBUG)
    small_game = MonkeyGames(RAW_SAMPLE)
    assert len(small_game.monkeys) == 4
    assert small_game.monkeys[0].items == [79, 98]
    assert small_game.monkeys[0].take_turn() == (2, [], 3, [500, 620])

    small_game = MonkeyGames(RAW_SAMPLE)
    small_game.play_round()
    assert small_game.monkeys[0].items == [20, 23, 27, 26]
    assert small_game.monkeys[1].items == [2080, 25, 167, 207, 401, 1046]
    assert small_game.monkeys[2].items == []
    assert small_game.monkeys[3].items == []

    for _ in range(19):
        small_game.play_round()
    assert small_game.get_monkey_business() == 10605

    my_game = MonkeyGames(RAW_INPUT)
    assert len(my_game.monkeys) == 8
    for _ in range(20):
        my_game.play_round()
    assert my_game.get_monkey_business() == 102399


def test_monkeys_part_2():
    small_game = MonkeyGames(RAW_SAMPLE, calm=False)
    small_game.play_round()
    assert [m.inspections for m in small_game.monkeys] == [2, 4, 3, 6]
    for _ in range(19):
        small_game.play_round()
    assert [m.inspections for m in small_game.monkeys] == [99, 97, 8, 103]
    small_game = MonkeyGames(RAW_SAMPLE, calm=False)
    for _ in range(10_000):
        small_game.play_round()
    assert small_game.get_monkey_business() == 2713310158

    my_game = MonkeyGames(RAW_INPUT, calm=False)
    for _ in range(10_000):
        my_game.play_round()
    assert my_game.get_monkey_business() == 23641658401
