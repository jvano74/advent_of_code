...
class Puzzle:
    ...
    With the Elves well on their way constructing the North Pole base, you turn your attention back to understanding
    ...
    "In programs where flow control is required, the instruction pointer can be bound to a register so that it can be
     manipulated directly. This way, setr/seti can function as absolute jumps, addr/addi can function as relative
    ...
    there are now six registers (numbered 0 through 5); the five not bound to the instruction pointer behave as normal.
    ...
    pointer, even if the value in the instruction pointer was just updated by an instruction.
    (Because of this, instructions must effectively set the instruction pointer to the instruction before the
    ...
    It turns out that this new information is already proving useful: the CPU in the device is not very powerful, and
    a background process is occupying most of its time. You dump the background process' declarations and instructions
    to a file (your puzzle input), making sure to use the names of the opcodes rather than the numbers.
    ...
    When executed, the following instructions are executed. Each line contains the value of the instruction
    pointer at the time the instruction started, the values of the six registers before executing the
    instructions (in square brackets), the instruction itself, and the values of the six registers after
    ...
    This is not an instruction, and so the value of the instruction pointer does not change during the processing
    of this line.

    The instruction pointer contains 0, and so the first instruction is executed (seti 5 0 1).
    It updates register 0 to the current instruction pointer value (0), sets register 1 to 5, sets the instruction
    pointer to the value of register 0 (which has no effect, as the instruction did not modify register 0), and then
...
with open('day_19_input.txt') as fp:
    ...
    INPUT_CODE = [[c if i == 0 else int(c) for i, c in enumerate(ln.split(' '))] for ln in INPUTS if ln[0] != '#']
    IP_REGESTER = 2
...
class Processor:
    ...
    binary_operations = {'add': operator.add, 'mul': operator.mul, 'div': operator.floordiv,
    ...
    def hx_analysis(self, multi_hit_min=2, skip_next_transitions=True):
        multi_hit = sorted(line for line, when_hit in self.hx_code_lines.items() if len(when_hit) >= multi_hit_min)
        transitions = {}
        for before, afters in self.hx_code_line_transitions.items():
            for after, state_change_count in afters.items():
                if before + 1 == after and state_change_count < multi_hit_min and skip_next_transitions:
                    pass
                else:
...
def test_processor():
    sample_code = [['seti', 5, 0, 1], ['seti', 6, 0, 2], ['addi', 0, 1, 0], ['addr', 1, 2, 3],
...
def test_puzzle2_processor():
    ...
    return_code = processor.run(code=INPUT_CODE, registers=[1, 0, 0, 0, 0, 0], max_steps=100_000)
    ...
    first_analysis = ([3, 4, 5, 6, 8, 9, 10, 11],
                      [((0, 17), 1),
                       ((3, 4), 12498),
                       ((4, 5), 12498),
                       ((5, 6), 12498),
    ...
    HACKED_CODE[3] = ['seti', 9, 1, 2]  # REMEMBER - jump to 1 before so to get 10 goto 9
    HACKED_CODE[10] = ['divr', 1, 5, 4]
    HACKED_CODE[11] = ['addr', 4, 0, 0]
    return_code = processor.run(code=HACKED_CODE, registers=[1, 0, 0, 0, 0, 0], max_steps=100_000)
    assert processor.registers[1] == 10551292

    # processor.track_hx = False
    # return_code = processor.run(code=HACKED_CODE, registers=[1, 0, 0, 0, 0, 0])
    hacked_analysis = ([2, 3, 10, 11, 12, 13, 14, 15],
                       [((0, 17), 1),
                        ((2, 3), 12498),
                        ((3, 10), 12498),
                        ((10, 11), 12498),
    ...
    assert return_code == (1, 'reached max_steps')
    assert processor.registers[0] == 78106251  # 78106251 and 102009186 not right, too high
