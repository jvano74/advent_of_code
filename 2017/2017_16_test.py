...
with open(Path(__file__).parent / "2017_16_input.txt") as fp:
...
def test_puzzle_shuffle():
    assert shuffle(INPUTS, "abcdefghijklmnop") == (
        "cknmidebghlajpfo",
        "abcdefghijklmnop",
    )
    ...
    assert (
        apply_perms("abcdefghijklmnop", [p_13, p_3], 1_000_000_000)
        == "lhafioejgmbkdcpn"
    )
    ...
    assert shuffle(INPUTS, "abcdefghijklmnop", split=True) == (
        "nbdfcpoejgamhilk",
        "lkinbmhjpgofaced",
    )
    # positional permutations
    perms_position = get_cycles("abcdefghijklmnop", "nbdfcpoejgamhilk")
    assert perms_position == [
        ["a", "n", "i", "j", "g", "o", "l", "m", "h", "e", "c", "d", "f", "p", "k"],
        ["b"],
    ]
    ...
    assert (
        apply_perms(apply_perms("abcdefghijklmnop", perms_position, 1), perms_labels, 1)
        == "cknmidebghlajpfo"
    )
    assert (
        apply_perms(
            apply_perms("abcdefghijklmnop", perms_position, 1_000_000_000),
            perms_labels,
            1_000_000_000,
        )
        == "cbolhmkgfpenidaj"
