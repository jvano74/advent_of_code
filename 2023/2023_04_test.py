...
RAW_SAMPLE = """Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19
Card 3:  1 21 53 59 44 | 69 82 63 72 16 21 14  1
Card 4: 41 92 73 84 69 | 59 84 76 51 58  5 54 83
Card 5: 87 83 26 28 32 | 88 30 70 12 93 22 82 36
...
def value_of_card(card, points=True):
    raw_winning, raw_on_card = card.split(": ")[1].split(" | ")
    winning = {n for n in raw_winning.split(" ") if n}
    score = 0
    for n in raw_on_card.split(" "):
...
def total_cards(duplication_list):
    cards = [1] * len(duplication_list)
    for pos, dup in enumerate(duplication_list):
        for offset in range(dup):
            new_card = pos + 1 + offset
            if new_card < len(duplication_list):
                cards[new_card] += cards[pos]
    return sum(cards)


def test_value_of_card():
    sample_matches = [value_of_card(card) for card in RAW_SAMPLE.split("\n")]
    assert sum(sample_matches) == 13
    sample_matches = [
        value_of_card(card, points=False) for card in RAW_SAMPLE.split("\n")
    ]
    assert total_cards(sample_matches) == 30

    my_matches = [value_of_card(card) for card in RAW_INPUT.split("\n")]
    assert sum(my_matches) == 32609
    my_matches = [value_of_card(card, points=False) for card in RAW_INPUT.split("\n")]
    assert total_cards(my_matches) == 14624680
