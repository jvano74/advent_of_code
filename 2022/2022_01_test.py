...
class Puzzle:
    ...
    To supply enough magical energy, the expedition needs to retrieve a minimum of fifty stars by December
    25th. Although the Elves assure you that the grove has plenty of fruit, you decide to grab any fruit
...
with open(Path(__file__).parent / "2022_01_input.txt") as fp:
    RAW_INPUT = fp.read()
...
def parse_input(raw_string):
    return [
        [int(f.strip()) for f in elf.split("\n")] for elf in raw_string.split("\n\n")
    ]


def test_parse_input():
    parsed_sample = parse_input(RAW_SAMPLE)
    assert parsed_sample == [
        [1000, 2000, 3000],
        [4000],
        [5000, 6000],
        [7000, 8000, 9000],
    ...
    assert max(sum(l) for l in parsed_sample) == 24000
    assert sum(sorted([sum(l) for l in parsed_sample])[-3:]) == 45000
    parsed_inputs = parse_input(RAW_INPUT)
    assert max(sum(l) for l in parsed_inputs) == 66616
    assert sum(sorted([sum(l) for l in parsed_inputs])[-3:]) == 199172
