...
with open('day_16_input.txt') as fp:
    ...
    INPUT_NEARBY_TICKETS = [[int(r) for r in line.strip().split(',')]
...
def test_ticket_validation():
    ...
    assert [test_validator.load_ticket(ticket) for ticket in SAMPLE_NEARBY_TICKETS] == [True, False, False, False]
    assert sum(test_validator.sum_of_invalid(ticket) for ticket in SAMPLE_NEARBY_TICKETS) == 71

    validator = TicketValidator(INPUT_RULES)
    assert sum(validator.sum_of_invalid(ticket) for ticket in INPUT_NEARBY_TICKETS) == 26988
