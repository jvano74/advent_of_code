class Puzzle:
    ...
    Once the data has been generated, you also need to create a checksum of that data. Calculate the checksum only
    ...
    exactly half as long as the original. If the length of the checksum is even, repeat the process until
...
def fill_disk(data, length):
    if len(data) >= length:
        return data[:length]
    rev_inv = ''.join(['1' if c == '0' else '0' for c in reversed(data)])
    return fill_disk(f'{data}0{rev_inv}', length)


def test_fill_disk():
    assert fill_disk('10', 15) == '100100101100100'
...
def test_sample():
    assert check_sum(fill_disk('10000', 20)) == '01100'


def test_puzzle():
    assert check_sum(fill_disk('01000100010010111', 272)) == '10010010110011010'

def test_puzzle2():
    # there are more efficient ways to calculat the check sum as we build the data
    # but the brute force approach is fast enough so good enough for now
    assert check_sum(fill_disk('01000100010010111', 35651584)) == '01010100101011100'
