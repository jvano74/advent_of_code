...
with open(Path(__file__).parent / "2018_16_input.txt") as fp:
    part_1_raw, part_2_raw = fp.read().split("\n\n\n\n")
    INPUT_PART_1 = [
        [ln.strip() for ln in ex.split("\n")] for ex in part_1_raw.split("\n\n")
    ]
    INPUT_PART_2 = [
        [int(d) for d in ln.strip().split(" ")] for ln in part_2_raw.split("\n")
    ]


def test_input():
    assert INPUT_PART_1[0] == [
        "Before: [0, 2, 0, 2]",
        "6 0 1 1",
        "After:  [0, 1, 0, 2]",
    ]
    assert INPUT_PART_1[-1] == [
        "Before: [1, 2, 2, 1]",
        "9 3 1 3",
        "After:  [1, 2, 2, 1]",
    ]
            ...
            for op_code_id, observations in unresolved_observations.items():
                for observation in observations:
                    ...
                    if op_code_id in possible_id_to_codes:
                        matching_op_codes = possible_id_to_codes[
                            op_code_id
                        ].intersection(matching_op_codes)
...
SAMPLE_OBSERVATION = Observation(
    operation=[9, 2, 1, 2], register_before=[3, 2, 1, 1], register_after=[3, 2, 2, 1]
)
...
def test_observations():
    ...
    more_than_2 = [
        1
        for obs in INPUT_PART_1
        if len(processor.check_observation(Observation.raw(obs))) > 2
    ]
