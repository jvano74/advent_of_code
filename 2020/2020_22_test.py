...
with open(Path(__file__).parent / "2020_22_input.txt") as fp:
    RAW = fp.read()
...
class Game:
    ...
    Before the game starts, split the cards so each player has their own deck (your puzzle input). Then, the
    game consists of a series of rounds: both players draw their top card, and the player with the higher-valued
    card wins the round. The winner keeps both cards, placing them on the bottom of their own deck so that the
    winner's card is above the other card. If this causes a player to have all of the cards, they win, and the
    game ends.
    ...
    This arrangement means that player 1's deck contains 5 cards, with 9 on top and 1 on the bottom; player 2's
    deck also contains 5 cards, with 5 on top and 10 on the bottom.

    The first round begins with both players drawing the top card of their decks: 9 and 5. Player 1 has the
    higher card, so both cards move to the bottom of player 1's deck such that 9 is above 5. In total, it
    ...
    --- Part Two ---
    You lost to the small crab! Fortunately, crabs aren't very good at recursion. To defend your honor as a
    Raft Captain, you challenge the small crab to a game of Recursive Combat.
    ...
    the sub-game). During this sub-game, the game that triggered it is on hold and completely unaffected;
...
def score_cards(cards):
    return sum(d * (i + 1) for i, d in enumerate(reversed(cards)))


def parse_raw(raw):
    player1, player2 = raw.split("\n\n")
    cards1 = [int(c.strip()) for c in player1.split("\n")[1:]]
    cards2 = [int(c.strip()) for c in player2.split("\n")[1:]]
...
def game(cards1, cards2):
    while True:
        if cards1[0] > cards2[0]:
            ...
            if len(cards2) == 0:
                return 1, cards1, score_cards(cards1)
        if cards1[0] < cards2[0]:
            cards2.append(cards2.pop(0))
            cards2.append(cards1.pop(0))
            if len(cards1) == 0:
                return 2, cards2, score_cards(cards2)


def test_game():
    cards1, cards2 = parse_raw(SAMPLE)
    assert game(cards1, cards2) == (2, [3, 2, 10, 6, 8, 5, 9, 4, 7, 1], 306)
    # now for real input
    cards1, cards2 = parse_raw(RAW)
    winner, _, score = game(cards1, cards2)
    assert score == 33772


def recursive_game(cards1, cards2, depth=0):
    rounds = set()
    print(f"starting game={depth} with {cards1} and {cards2}")
    while len(cards1) > 0 and len(cards2) > 0:
        if (f"{cards1}", f"{cards2}") in rounds:
            return 1, cards1, cards2

        rounds.add((f"{cards1}", f"{cards2}"))
        card_p1 = cards1.pop(0)
        card_p2 = cards2.pop(0)
        if card_p1 <= len(cards1) and card_p2 <= len(cards2):
            winner, _, _ = recursive_game(cards1[:card_p1], cards2[:card_p2], depth + 1)
        ...
        if winner == 1:
            cards1.append(card_p1)
            cards1.append(card_p2)
            print(f"game={depth} round winner 1, now {cards1} and {cards2}")
        else:
            cards2.append(card_p2)
            cards2.append(card_p1)
            print(f"game={depth} round winner 2, now {cards1} and {cards2}")
    if len(cards1) == 0:
...
def test_recursive_game():
    ...
    print()
    cards1, cards2 = parse_raw(SAMPLE)
    winner, cards1, cards2 = recursive_game(cards1, cards2)
    assert winner == 2
    assert score_cards(cards2) == 291
    # now for real input
    cards1, cards2 = parse_raw(RAW)
    winner, cards1, cards2 = recursive_game(cards1, cards2)
    print(winner)
    print(cards1, score_cards(cards1))
    print(cards2, score_cards(cards2))
    assert winner == 1
    assert score_cards(cards1) == 35070
