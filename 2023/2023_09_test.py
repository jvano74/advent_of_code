...
class Puzzle:
    ...
    Since these values aren't all zero, repeat the process: the values differ by
    0 at each step, so the next sequence is 0 0 0 0. This means you have enough
...
RAW_REPORT = [[0, 3, 6, 9, 12, 15], [1, 3, 6, 10, 15, 21], [10, 13, 16, 21, 30, 45]]


def calculate_delta_set(sequence):
    deltas = [sequence]
    while True:
        if len(sequence) < 2:
            return deltas
        ...
        for next_element in sequence[1:]:
            next_delta = next_element - element
            next_sequence.append(next_delta)
            if next_delta != 0:
                all_zero = False
            element = next_element
        sequence = next_sequence
        deltas.append(sequence)
        if all_zero:
            return deltas


def next_value(delta_set, rev=False):
    value = 0
    while delta_set:
        next_delta = delta_set.pop()
        if rev:
            value = next_delta[0] - value
        else:
            value += next_delta[-1]
    return value
...
def test_delta_sets():
    assert sum(next_value(calculate_delta_set(seq)) for seq in RAW_REPORT) == 114
    assert (
        sum(next_value(calculate_delta_set(seq), rev=True) for seq in RAW_REPORT) == 2
    )
    assert sum(next_value(calculate_delta_set(seq)) for seq in RAW_INPUT) == 1702218515
    assert (
        sum(next_value(calculate_delta_set(seq), rev=True) for seq in RAW_INPUT) == 925
    )
