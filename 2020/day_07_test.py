...
def split_line(line):
    ...
    return outer_bag, {' '.join(c.split(' ')[1:]): int(c.split(' ')[0]) for c in inner_bags if c.split(' ')[0] != 'no'}


def test_split_line():
    assert split_line(SAMPLE[0]) == ('light red', {'bright white': 1, 'muted yellow': 2})
...
def test_find_total_inner_bags():
    ...
    assert find_total_inner_bags(bag_contains, 'shiny gold') == 5956 # too low 1906, 1907
