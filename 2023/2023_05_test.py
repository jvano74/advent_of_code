...
def create_transforms(raw_mapping):
    transforms = []
    for raw_range in raw_mapping.split("\n")[1:]:
        y_0, x_0, delta = [int(digit) for digit in raw_range.split(" ")]
        x_1 = x_0 + delta
        b = y_0 - x_0
        transforms.append((x_0, x_1, b))
    transforms.sort()
    return transforms


def apply_transform_to_point(x, transforms):
    for x_0, x_1, b in transforms:
        if x in range(x_0, x_1):
            return x + b
    return x


def apply_transform_to_interval(x_min, x_max, transforms):
    x = x_min
    intervals = []
    for x_0, x_1, b in transforms:
        if x > x_1:
        ...
        if x in range(x_0, x_1):
            if x_1 >= x_max:
                intervals.append((x + b, x_max + b))
                return intervals
            else:
                intervals.append((x + b, x_1 + b))
                x = x_1
        if x < x_0:
            if x_max < x_0:
                intervals.append((x, x_max))
                return intervals
            intervals.append((x, x_0))
            if x_max <= x_1:
                intervals.append((x_0 + b, x_max + b))
                return intervals
            intervals.append((x_0 + b, x_1 + b))
            x = x_1
    if x < x_max:
        intervals.append((x, x_max))
    return intervals
...
def coaless_intervals(intervals):
    if len(intervals) < 2:
        return intervals
    intervals.sort()
    new_interval_list = []
    ...
    for a, b in intervals:
        if a < x_1:
            x_1 = max(x_1, b)
            continue
        new_interval_list.append((x_0, x_1))
        x_0, x_1 = a, b
    new_interval_list.append((x_0, x_1))
    return new_interval_list


def extract_seeds_and_transforms(raw_data):
    steps = raw_data.split("\n\n")
    seeds = [int(d) for d in steps[0].replace("seeds: ", "").split(" ")]
    transforms = [create_transforms(step) for step in steps[1:]]
    return seeds, transforms


def apply_all_transforms_to_points(seeds, transforms):
    results = seeds[:]
    for transform in transforms:
        new_results = [apply_transform_to_point(seed, transform) for seed in results]
        results = new_results[:]
    return results


def apply_all_transforms_to_intervals(intervals, transforms):
    results = intervals[:]
    for transform in transforms:
        new_results = []
        for a, b in results:
            new_results.extend(
                coaless_intervals(apply_transform_to_interval(a, b, transform))
            )
        results = coaless_intervals(new_results)
    return results


def test_transforms():
    # part 1
    seeds, transforms = extract_seeds_and_transforms(RAW_SAMPLE)
    assert min(apply_all_transforms_to_points(seeds, transforms)) == 35

    # part 2
    intervals = [(x_0, x_0 + delta) for x_0, delta in zip(seeds[::2], seeds[1::2])]
    assert intervals == [(79, 93), (55, 68)]
    assert coaless_intervals(intervals) == [(55, 68), (79, 93)]
    final_locations = apply_all_transforms_to_intervals(intervals, transforms)
    assert final_locations[0][0] == 46

    # my_data
    seeds, transforms = extract_seeds_and_transforms(RAW_INPUT)
    assert min(apply_all_transforms_to_points(seeds, transforms)) == 331445006
    intervals = [(x_0, x_0 + delta) for x_0, delta in zip(seeds[::2], seeds[1::2])]
    final_locations = apply_all_transforms_to_intervals(intervals, transforms)
    assert final_locations[0][0] == 6472060
