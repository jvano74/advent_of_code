...
with open(Path(__file__).parent / "2020_19_input.txt") as fp:
    RAW = fp.read()
    INPUT_RULES, INPUT_MESSAGES = RAW.split("\n\n")
    INPUT_RULES = INPUT_RULES.split("\n")
    INPUT_MESSAGES = INPUT_MESSAGES.split("\n")
...
class Puzzle:
    ...
    image of a sea monster! Unfortunately, the connection to the satellite is having problems, and
    ...
    the rules so you can determine which are valid and which are corrupted. Including the rules
    and the messages together, this might look like:
    ...
    above example, ababbb and abbbab match, but bababa, aaabbb, and aaaabbb do not, producing
    the answer 2. The whole message must match all of rule 0; there can't be extra unmatched
    characters in the message. (For example, aaaabbb might appear to match rule 0 above, but
    ...
    Fortunately, many of the rules are unaffected by this change; it might help to start by looking at which
    rules always match the same set of values and how those rules (especially rules 42 and 31) are used by
...
class Rules:
    ...
    def __init__(self, rules_list, new_rule_list=None):
        if new_rule_list is None:
            new_rule_list = {}
        self.new_rule_list = new_rule_list
        self.rules = {}
        self.higher_order_rules = {}

        for rule in rules_list:
            rule_id, conditions = rule.split(": ")
            if rule_id in self.new_rule_list:
                # pre wrap your rule tokens
                # conditions = '#'.join(self.new_rule_list[rule].split(' '))
                # conditions = f'#{conditions}#'
                self.higher_order_rules[rule_id] = self.new_rule_list[rule_id]
            elif conditions.count('"') > 0:
                if conditions.count(" ") > 0:
                    raise Exception(
                        f"Atom found in component list {rule_id}: {conditions}"
                    )
                self.rules[rule_id] = f"{conditions[1:-1]}"
            else:
                conditions = "#".join(conditions.split(" "))
                conditions = f"#{conditions}#"
                if conditions.count("|") > 0:
                    conditions = f"({conditions})"
                self.higher_order_rules[rule_id] = conditions
        back_links = self.map_back_links()

        while len(back_links) > 0:
            snapshot_of_known_rules = set(self.rules).intersection(back_links)
            for known_rule_id in snapshot_of_known_rules:
                for back_rule_id in back_links[known_rule_id]:
                    if back_rule_id in self.higher_order_rules:
                        conditions = self.higher_order_rules[back_rule_id]
                        conditions = conditions.replace(
                            f"#{known_rule_id}#", f"#{self.rules[known_rule_id]}#"
                        )
                        if len(re.findall(r"#\d+#", conditions)) > 0:
                            self.higher_order_rules[back_rule_id] = conditions
                        else:
                            self.higher_order_rules.pop(back_rule_id, None)
                            self.rules[back_rule_id] = conditions.replace("#", "")
            back_links = self.map_back_links()
        for rule_id in self.rules:
            self.rules[rule_id] = f"^{self.rules[rule_id]}$"

    def map_back_links(self):
        back_links = defaultdict(set)
        for rule in self.higher_order_rules:
            condition = self.higher_order_rules[rule]
            for unknown in re.findall(r"\d+", condition):
                back_links[unknown].add(rule)
        return back_links
...
SAMPLE_MESSAGES = ["ababbb", "bababa", "abbbab", "aaabbb", "aaaabbb"]


def test_sample_rules():
    sample_rules = Rules(SAMPLE_RULES)
    print()
    print(sample_rules.rules["0"])
    print()
    assert re.match(sample_rules.rules["0"], "ababbb")
    assert not re.match(sample_rules.rules["0"], "bababa")
    assert re.match(sample_rules.rules["0"], "abbbab")
    assert not re.match(sample_rules.rules["0"], "aaabbb")
    assert not re.match(sample_rules.rules["0"], "aaaabbb")


def test_puzzle_rules():
    puzzle_rules = Rules(INPUT_RULES)
    rule_0 = re.compile(puzzle_rules.rules["0"])
    matching_msg = 0
    for msg in INPUT_MESSAGES:
        if rule_0.match(msg):
            matching_msg += 1
    assert matching_msg == 139


def test_puzzle_rules2():
    #  8: 42 becomes 8: 42 | 42 8
    #     for 8 we can check this via 8: (42)+
    # 11: 42 31 becomes 11: 42 31 | 42 11 31
    #     this is a bit harder because we need the same number of counts
    #     to do a quick brute force lets try
    # 11: 42 31 | 4242 31 31 | 42 42 42 31 31 31
    puzzle_rules = Rules(
        INPUT_RULES,
        {
            "8": "(#42#)+",
    ...
    for msg in INPUT_MESSAGES:
        if rule_0.match(msg):
            matching_msg += 1
    assert matching_msg == 289
    # 309 is too high? - was missing () around expanded 11...
    # worked but still not in general becuase new rule 11 could repeat more than 5 times...
