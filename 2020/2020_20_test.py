...
class Tile:
    ...
    def get_side(self, external_side):
        ...
        if self.flipped:
            ...
            internal_side = position_to_direction[
                (external_position - self.cc_rotation) % 4
            ]
...
class Board:
    def __init__(self, raw, width=0, height=0):
        ...
        self.orientations = set(
            (r, False if t == 0 else True) for r in range(4) for t in range(2)
        )
        ...
        if starting_board is None:
            for tile_id in self.tiles.keys():
                ...
                for cc_rotation, flipped in self.orientations:
                    frontier.append(
                        ({(0, 0): (tile_id, cc_rotation, flipped)}, free_tiles.copy())
                    )
        else:
            used_tiles = set(t_id for t_id, _, _ in starting_board.values())
            free_tiles = set(
                t_id for t_id in self.tiles.keys() if t_id not in used_tiles
            )
        ...
        while len(frontier) > 0:
            ...
            if len(working_board) == len(self.tiles):
                ...
                for tile_id in free_tiles:
                    tile_id_fit = 0
                    for cc_rotation, flipped in self.orientations:
                        if self.new_piece_fits(
                            working_board, next_location, tile_id, cc_rotation, flipped
                        ):
                            tile_id_fit += 1
                            new_board = working_board.copy()
                            next_free_tiles = set(
                                t_id for t_id in free_tiles if t_id != tile_id
                            )
...
def update_sea_monsters(image_as_dict, to_screen=False):
    ...
    for rot, flip in orientations:
        new_image_dict = rotate_and_flip(
            image_as_dict, image_x_max, image_y_max, rot, flip
        )
...
with open(Path(__file__).parent / "2020_20_sample.txt") as fp:
    SAMPLE = fp.read()
...
def test_board_new_piece_fits():
    ...
    cc_rotation = 2
    flipped = True
    assert sample_board.new_piece_fits(
        working_board, location, tile_id, cc_rotation, flipped
    )
    working_board = {
        (0, 0): ("1951", 2, True),
        (1, 0): ("2311", 2, True),
...
with open(Path(__file__).parent / "2020_20_input.txt") as fp:
    INPUT = fp.read()
