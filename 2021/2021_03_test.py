...
with open(Path(__file__).parent / "2021_03_input.txt") as fp:
    INPUTS = [line.strip() for line in fp]
assert len(INPUTS) == 1000


def find_gamma_epsilon(inputs):
    total = len(inputs[0]) * [0]
    for bs in inputs:
        for i, d in enumerate(bs):
            if d == "1":
                total[i] += 1
    gamma = "".join(["1" if 2 * d > len(inputs) else "0" for d in total])
    epsilon = "".join(["0" if 2 * d > len(inputs) else "1" for d in total])
    return gamma, epsilon


def test_find_gamma_epsilon():
    assert find_gamma_epsilon(SAMPLE) == ("10110", "01001")
    assert int("10110", 2) * int("01001", 2) == 198
    assert find_gamma_epsilon(INPUTS) == ("011000011101", "100111100010")
    assert int("011000011101", 2) * int("100111100010", 2) == 3959450


def sub_sorter(inputs, position, keep):
    if len(inputs) == 1:
        return inputs[0]
    zeros = []
    ones = []
    ...
    if keep == "most":
        if len(ones) > len(zeros):
            return sub_sorter(ones, position + 1, keep)
        elif len(ones) < len(zeros):
            return sub_sorter(zeros, position + 1, keep)
        else:
            return sub_sorter(ones, position + 1, keep)
    elif keep == "least":
        if len(ones) > len(zeros):
...
def life_support_rating(inputs):
    oxygen_generator = int(sub_sorter(inputs, 0, "most"), 2)
    co2_scrubber = int(sub_sorter(inputs, 0, "least"), 2)
    return oxygen_generator * co2_scrubber


def test_life_support_rating():
    assert life_support_rating(SAMPLE) == 230
    assert life_support_rating(INPUTS) == 7440311
