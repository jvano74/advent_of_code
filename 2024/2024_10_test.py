...
with open(Path(__file__).parent / "2024_10_input.txt") as fp:
    ...
    def __init__(self, map: List[str]):
        ...
        for y, line in enumerate(map):
            ...
            for x, c in enumerate(line):
                ...
                pt = Pt(x, y)
                self.elevation[pt] = int(c)
                if c == "0":
                    self.trail_heads.add(pt)

    def find_trails(self, end_only=True):
        hiking = [[loc] for loc in self.trail_heads]
        trails = defaultdict(list)
        while hiking:
            path = hiking.pop()
            for next_step in [Pt(1, 0), Pt(0, -1), Pt(-1, 0), Pt(0, 1)]:
                next_location = path[-1] + next_step
                if next_location in self.elevation and (
                    self.elevation[next_location] == len(path)
                ):
                    new_path = path[:]
                    new_path.append(next_location)
                    if len(new_path) < 10:
                        hiking.append(new_path)
                    else:
                        trails[new_path[0]].append(new_path)
        if end_only:
            return {
                head: {trail[-1] for trail in trails}
                for (head, trails) in trails.items()
            }
        return trails
...
def test_find_trail():
    ...
    trails = sample.find_trails()
    assert sum(len(trails) for trails in trails.values()) == 36
    trails = sample.find_trails(end_only=False)
    assert sum(len(trails) for trails in trails.values()) == 81
    my_map = Map(MY_MAP)
    my_trails = my_map.find_trails()
    assert sum(len(my_trails) for my_trails in my_trails.values()) == 778
    my_trails = my_map.find_trails(end_only=False)
    assert sum(len(my_trails) for my_trails in my_trails.values()) == 1925
