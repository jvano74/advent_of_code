...
class Monad:
        ...
        for pointer, raw_instruction in enumerate(self.program):
            ...
            if cmd == 'inp':
                ...
                if value.isdigit() or value[0] == '-':
                    ...
                    self.variables[var] = 1 if self.variables[var] == self.variables[value] else 0
    ...
    def extract_monads(self):
        ...
        m_settings.append(m_vars)
        return ['| '.join(f'{i: <9}' for i in monads[i]) for i in range(max_line+1)], m_settings
...
def my_function(input_string):
    ...
    m_settings = [[1, 11, 14], [1, 13, 8], [1, 11, 4], [1, 10, 10], [26, -3, 14], [26, -4, 10], [1, 12, 4],
                  [26, -8, 14], [26, -3, 1], [26, -12, 6], [1, 14, 0], [26, -6, 9], [1, 11, 13], [26, -12, 12]]
    z = 0
