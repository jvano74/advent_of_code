...
SAMPLE_2 = [(71530, 940200)]
# MY_INPUT = {"Time": 51699878, "Distance": 377117112241505}
MY_INPUT = [(51, 377), (69, 1171), (98, 1224), (78, 1505)]
MY_INPUT_2 = [(51699878, 377117112241505)]


def ways_to_beat(time, dist, formula=True):
    if formula:
        return time - 2 * floor((time - sqrt(time**2 - 4 * dist)) / 2) - 1
    winning = 0
...
def total_wins(races):
    ways_to_win = 1
    for time, dist in races:
        ways_to_win *= ways_to_beat(time, dist)
    return ways_to_win
...
def test_ways_to_beat():
    ...
    assert total_wins(SAMPLE) == 288
    assert total_wins(SAMPLE_2) == 71503
    assert total_wins(MY_INPUT) == 131376
    assert total_wins(MY_INPUT_2) == 34123437
