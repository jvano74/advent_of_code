...
class Puzzle:
    ...
    it point at 99. Similarly, turning the dial right from 99 one click makes it
    point at 0.
...
with open(Path(__file__).parent / "2025_01_input.txt") as fp:
    RAW_INPUT = fp.read()
...
def rotate_dial(
    rotations: List, starting_number: int = 50, enhanced: bool = False
) -> int:
    dial = starting_number
    zeros = 0
    tracking = []
    for rotation in rotations:
        ...
        if enhanced:
            while dial < 0:
                tracking.append(f"pass {zeros=} during {rotation=}")
                dial += 100
                if last_dial == 0:
                    last_dial = None
                else:
        ...
        if dial == 0:
            zeros += 1
            tracking.append(f"end {zeros=} on {rotation=}")
    return zeros


def test_basic_password():
    assert rotate_dial(SAMPLE) == 3
    assert rotate_dial(RAW_INPUT.split("\n")) == 1145


def test_enhanced_password():
    assert rotate_dial(SAMPLE, enhanced=True) == 6
    assert rotate_dial(RAW_INPUT.split("\n"), enhanced=True) == 6561
    # Note, first guess was 6580 which was double counts going left starting on 0
    # fixed and then got 6561
