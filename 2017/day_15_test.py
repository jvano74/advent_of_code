class Puzzle:
    ...
    Here, you can see that the lowest (here, rightmost) 16 bits of the third value match: 1110001101001010. Because
    of this one match, after processing these five pairs, the judge would have added only 1 to its total.
    ...
    occasionally handing an acceptable value to the judge, and otherwise working through the same sequence of
...
def generator(factor, current, condition=0):
    """
    previous value * factor div by 2147483647
    """
    while True:
...
def judge(starting_a, starting_b, count, cond_a=0, cond_b=0):
    generator_a = generator(16807, starting_a, cond_a)
    generator_b = generator(48271, starting_b, cond_b)
    total = 0
    for _ in range(count):
        if (next(generator_a) - next(generator_b)) % 65536 == 0:
            total += 1
    return total


def test_judge():
    assert judge(65, 8921, 2) == 0
    assert judge(65, 8921, 3) == 1
    assert judge(65, 8921, 5) == 1

    assert judge(618, 814, 40_000_000) == 577
    assert judge(618, 814, 5_000_000, 4, 8) == 316