...
def count_increase(depths):
    return sum([1 if depths[i] > depths[i-1] else 0 for i, d in enumerate(depths) if i > 0])


def count_window_increase(depths):
    return sum([1 if depths[i] > depths[i-3] else 0 for i, d in enumerate(depths) if i > 2])
