class Puzzle:
    ...
    by diagonals: starting with the first row with an empty first box, the codes are filled in diagonally up and
    to the right. This process repeats until the infinite paper is covered. So, the first few codes are filled in
    in this order:
    ...
    it by 252533 to get 5088824049625. Then, divide that by 33554393, which leaves a remainder of 31916031.
...
def test_code_generator():
    ...
    row, col = 0, 0
    assert next(codes) == (20151125, 1, 1)
    assert next(codes) == (31916031, 2, 1)
    assert next(codes) == (18749137, 1, 2)
    assert next(codes) == (16080970, 3, 1)
    assert next(codes) == (21629792, 2, 2)
    while (row, col) != (4, 6):
        value, row, col = next(codes)
    assert value == 31527494


def test_answer():
    codes = code_generator()
    row, col = 0, 0
    while (row, col) != (3010, 3019):
        value, row, col = next(codes)
    assert value == 8997277
