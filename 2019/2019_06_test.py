...
class Galaxy(defaultdict):
    def transfer_distance(self, you: str, santa: str):
        ...
        return 2 + self.transfer_distance(
            self[you].parent.name, self[santa].parent.name
        )
...
def test_submission():
    with open(Path(__file__).parent / "2019_06_input.txt", "r") as f:
    ...
    assert (
        galaxy.transfer_distance(galaxy["YOU"].parent.name, galaxy["SAN"].parent.name)
        == 283
    )
