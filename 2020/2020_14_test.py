...
with open(Path(__file__).parent / "2020_14_input.txt") as fp:
...
class Memory:
    ...
    After a brief inspection, you discover that the sea port's computer system uses a strange bitmask system in
    its initialization program. Although you don't have the correct decoder chip handy, you can emulate it in software!
    ...
    def load_instructions(self, instructions):
        for line in instructions:
            ...
            if task == "mask":
                self.update_bitmask(value)
            else:
                location = int(task[4:-1])
                value = int(value)
                self.write_to_memory(location, value)

    def update_bitmask(self, mask):
        multiplier = 1
        on_mask = 0
        off_mask = 0
    ...
    def apply_mask(self, value):
        return (value | self.on_mask) & self.off_mask

    def write_to_memory(self, location, value):
        self.memory[location] = self.apply_mask(value)

    def mem_sum(self):
        return sum(v for _, v in self.memory.items())
...
def test_sample():
    sample_mem = Memory()
    sample_mem.load_instructions(SAMPLE)
    assert sample_mem.memory[7] == 101
    assert sample_mem.memory[8] == 64
    assert sample_mem.mem_sum() == 165


def test_submission():
    doc_mem = Memory()
    doc_mem.load_instructions(INPUT)
    assert doc_mem.mem_sum() == 9615006043476
...
class MemoryV2:
    ...
    def apply_mask(self, value):
        ...
        for c in reversed(self.mask):
            if c == "1":
                mask_list = [(m | multiplier) for m in mask_list]
            elif c == "X":
                if value & multiplier:
                    value ^= multiplier
                for i in range(len(mask_list)):
                    mask_list.append(mask_list[i] | multiplier)
            multiplier *= 2
        self.mask_list = mask_list
        return [value | mask for mask in self.mask_list]

    def write_to_memory(self, orig_location, value):
        for location in self.apply_mask(orig_location):
...
def test_sample_v2():
    print()
    sample_mem = MemoryV2()
    sample_mem.load_instructions(SAMPLE2)
    assert sample_mem.mem_sum() == 208


def test_submission_v2():
    doc_mem = MemoryV2()
    doc_mem.load_instructions(INPUT)
    assert doc_mem.mem_sum() == 4275496544925
