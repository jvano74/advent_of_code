...
with open(Path(__file__).parent / "2021_18_input.txt") as fp:
...
def test_reduce_snail():
    assert (
        reduce_snail("[[[[[4,3],4],4],[7,[[8,4],9]]],[1,1]]")
        == "[[[[0,7],4],[[7,8],[6,0]]],[8,1]]"
    )
...
def test_add_snail_list():
    assert (
        add_snail_list(["[1,1]", "[2,2]", "[3,3]", "[4,4]"])
        == "[[[[1,1],[2,2]],[3,3]],[4,4]]"
    )
    assert (
        add_snail_list(
...
def test_find_magnitude():
    ...
    assert (
        find_magnitude("[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]") == 3488
    )
...
def test_submission():
    ...
    assert (
        max(find_magnitude(add_snail(a, b)) for a in INPUTS for b in INPUTS if a != b)
        == 4685
    )
