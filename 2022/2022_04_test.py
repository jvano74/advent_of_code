...
with open(Path(__file__).parent / "2022_04_input.txt") as fp:
    RAW2 = fp.read()


def contains(ab, full=True):
    a, b = ab.split(",")
    a = [int(n) for n in a.split("-")]
    b = [int(n) for n in b.split("-")]
    if a[0] == b[0]:
        return True
    if b[0] < a[0]:  # swap so a[0] < b[0]
        a, b = b, a
    if a[1] >= b[1]:
        return True
    if full:
...
def test_contains():
    assert contains(RAW.split("\n")[0]) is False
    assert contains("2-8,3-7") is True
    assert sum(1 if contains(x) else 0 for x in RAW.split("\n")) == 2
    assert sum(1 if contains(x, full=False) else 0 for x in RAW.split("\n")) == 4
    assert sum(1 if contains(x) else 0 for x in RAW2.split("\n")) == 498
    assert sum(1 if contains(x, full=False) else 0 for x in RAW2.split("\n")) == 859
