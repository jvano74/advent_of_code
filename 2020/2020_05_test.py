...
class Puzzle:
    ...
    You aren't sure which seat is yours, and all of the flight attendants are busy with the flood of
...
with open(Path(__file__).parent / "2020_05_input.txt") as f:
    INPUTS = [line.strip() for line in f]


def string_to_id(seat_string):
    return int(
        seat_string.replace("F", "0")
        .replace("B", "1")
        .replace("L", "0")
        .replace("R", "1"),
        2,
...
def find_seat(used_seat_ids, max=913):
    open_seats = set(range(max))
    for p in used_seat_ids:
        open_seats.discard(p)
    for remaining in open_seats:
        if remaining + 1 not in open_seats and remaining - 1 not in open_seats:
            return remaining


def test_string_to_id():
    assert string_to_id("BFFFBBFRRR") == 567
    assert max(string_to_id(s) for s in SAMPLE) == 820
    assert max(string_to_id(s) for s in INPUTS) == 913
    assert find_seat(string_to_id(s) for s in INPUTS) == 717
