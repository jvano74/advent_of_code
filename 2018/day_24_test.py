            ...
            for s in specials[0].split(';'):
                ...
                if s.count('immune to') > 0:
                    for e in s.split('immune to ')[1].split(', '):
                        immunity.add(e)
        attack_damage = int(re.findall(r'with an attack that does (\d+)', line)[0]) + boost
...
def test_group():
    sample = '8808 units each with 5616 hit points (immune to cold; weak to radiation) ' \
    ...
    sample2 = '3098 units each with 19840 hit points (weak to bludgeoning, cold) ' \
...
class ImmuneSystem:
    ...
    def count(self):
        return sum(g.units for g in self.groups if g.type == 'immune'), \
               sum(g.units for g in self.groups if g.type != 'immune')
    ...
    def target_selection_order(self):
        ...
        """
        order = [(g.effective_power(), g.initiative, i)
    ...
    def select_target(self):
        ...
        for attacker_position in self.target_selection_order():
            ...
            possible_targets = {(g.damage_multiplier(attacker), g.effective_power(), g.initiative, i)
                                for i, g in enumerate(self.groups)
                                if i not in already_picked and
                                g.type != attacker.type and
                                g.units > 0 and
                                g.damage_multiplier(attacker) > 0}
    ...
    def attack_order(self):
        ...
        """
        order = [(self.groups[ag].initiative, ag, tg) for ag, tg in self.select_target()]
        sorted_order = sorted(order, reverse=True)
...
SAMPLE_IMMUNE = ['17 units each with 5390 hit points (weak to radiation, bludgeoning) ' +
...
SAMPLE_INFECTION = ['801 units each with 4706 hit points (weak to radiation) ' +
