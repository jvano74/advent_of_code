...
with open(Path(__file__).parent / "2025_03_input.txt") as fp:
    RAW_INPUT = fp.read().split("\n")

SAMPLE = ["987654321111111", "811111111111119", "234234234234278", "818181911112111"]


def max_bank(bank: str, size: int = 2) -> int:
    if len(bank) < size:
        return 0
    result = [0] * size
    min_pos = 0
    for max_pos in range(size):
        pos_max_pos = len(bank) - size + max_pos
        pos_max_val = int(bank[pos_max_pos])
        for pos in range(pos_max_pos, min_pos - 1, -1):
            if int(bank[pos]) >= pos_max_val:
                pos_max_pos = pos
                pos_max_val = int(bank[pos])
        result[max_pos] = str(pos_max_val)
        min_pos = pos_max_pos + 1
    return int("".join(result))


def test_check_range():
    assert max_bank("987654321111111") == 98
    assert max_bank("811111111111119") == 89
    assert max_bank("234234234234278") == 78
    assert max_bank("818181911112111") == 92
    assert sum(max_bank(bank) for bank in SAMPLE) == 357
    assert sum(max_bank(bank, size=12) for bank in SAMPLE) == 3121910778619


def test_my_check_range():
    assert sum(max_bank(bank) for bank in RAW_INPUT) == 17330
    assert sum(max_bank(bank, size=12) for bank in RAW_INPUT) == 171518260283767
