...
time and flashes brightly for a moment when its energy is full. Although your lights are off,
maybe you could navigate through the cave without disturbing the octopuses if you could predict
...
class Octogrid:
    def __init__(self, starting_levels):
        self.level = {}
        for y, row in enumerate(starting_levels):
            for x, l in enumerate(row):
                self.level[Pt(x, y)] = int(l)
    ...
    def run_to_sync(self):
        steps = 0
        flashes = 0
        while flashes < len(self.level):
            flashes = self.step()
            steps += 1
        return steps

    def step(self):
        deltas = {Pt(1, 0), Pt(1, 1), Pt(0, 1), Pt(-1, 1),
                  Pt(-1, 0), Pt(-1, -1), Pt(0, -1), Pt(1, -1)}
        flashers = []
        reset = set()
        for pt in self.level:
            self.level[pt] += 1
            if self.level[pt] > 9:
                flashers.append(pt)
        while len(flashers) > 0:
            pt = flashers.pop()
            reset.add(pt)
            for nbh in [pt + d for d in deltas if pt + d in self.level]:
                self.level[nbh] += 1
                if nbh in reset:
                    pass
                elif nbh in flashers:
                    pass
                elif self.level[nbh] > 9:
                    flashers.append(nbh)
        for pt in reset:
            self.level[pt] = 0
        return len(reset)
...
def test_octogrid():
    sample_octogrid = Octogrid(SAMPLE)
    assert sample_octogrid.step() == 9
    assert sample_octogrid.step() == 0
    sample2_octogrid = Octogrid(SAMPLE2)
    assert sample2_octogrid.run(100) == 1656
    puzzle_octogrid = Octogrid(INPUT)
    assert puzzle_octogrid.run(100) == 1755
    # part 2
    sample2_octogrid = Octogrid(SAMPLE2)
    assert sample2_octogrid.run_to_sync() == 195
    puzzle_octogrid = Octogrid(INPUT)
    assert puzzle_octogrid.run_to_sync() == 212
