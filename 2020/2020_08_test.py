...
class Puzzle:
    ...
    the in-flight menu for one of those drinks that come with a little umbrella, you are interrupted by the
    ...
    next instruction to the other acc +1 near the bottom. After it increases the accumulator from 1 to 2,
...
class Program:
    def __init__(self, program):
        ...
        self.count = 0
        self.visited = [0]
        self.first_loop_bacl = -1

    def un_tick(self, head):
        pass

    def tick(self) -> int:
        self.count += 1
        operation, argument = self.inst[self.head]
        if operation == "nop":
            self.head += 1
        if operation == "acc":
            self.acc += argument
        ...
        if self.head in self.visited:
            self.fault = True
            self.first_loop_bacl = self.count
        self.visited.append(self.head)

        if self.head >= len(self.inst):
            self.fault = True
...
with open(Path(__file__).parent / "2020_08_input.txt") as fp:
    RAW = [instruction.strip().split(" ") for instruction in fp]
    SRC = [(op, int(arg)) for op, arg in RAW]
...
def test_sample():
    ...
    PONG = Program(SRC)
    PONG.run()
    assert PONG.acc == 2080


def test_part2():
    for pos, inst in enumerate(SAMPLE):
        NEW = SAMPLE[:]
        if inst[0] == "nop":
            NEW[pos] = ("jmp", inst[1])
        elif inst[0] == "jmp":
