...
with open(Path(__file__).parent / "2019_17_input.txt") as fp:
    raw = fp.read()
...
class Image:
    ...
    def find_intersections(self):
        for pt in self.scaffold:
            if {Point(pt.x + d.x, pt.y + d.y) for d in self.DELTAS}.issubset(
                self.scaffold
            ):
...
def test_submission():
    ...
    assert (
        image.find_path(Point(0, -16), Point(0, 1)) == "R,6,L,8,R,8,"
        "R,6,L,8,R,8,"
        "R,4,R,6,R,6,R,4,R,4,"
        "L,8,R,6,L,10,L,10,"
        "R,4,R,6,R,6,R,4,R,4,"
