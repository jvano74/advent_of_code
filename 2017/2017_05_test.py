...
class Puzzle:
    ...
    (1) 3  0  1  -3  - jump with offset 0 (that is, don't jump at all). Fortunately, the instruction is then
                       incremented to 1.
     2 (3) 0  1  -3  - step forward because of the instruction we just modified. The first instruction is
...
def test_run_to_exit():
    assert run_to_exit(SAMPLE) == 5
    assert run_to_exit(INPUTS) == 372671
