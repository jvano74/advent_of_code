...
class Puzzle:
    ...
    you to a shelf of goggles and hard hats (you quickly find some that fit) and
    then further into the facility. At one point, you pass a button with a faint
...
with open(Path(__file__).parent / "2023_15_input.txt") as fp:
        ...
        for i in range(len(boxes[box])):
            ...
            if a == label:
                if add:
                    ...
                    break
        if add:
            boxes[box].append((label, int(f_len)))

    power = 0
    for box, lenses in boxes.items():
        for i, (_, f_len) in enumerate(lenses):
            power += (box + 1) * (i + 1) * f_len
...
def test_hash():
    assert hash("HASH") == 52
    assert sum(hash(s) for s in SAMPLE) == 1320
    assert hashmap(SAMPLE) == 145
    assert sum(hash(s) for s in RAW_INPUT) == 510801
    assert hashmap(RAW_INPUT) == 212763
