...
class Puzzle:
    ...
    "Programs these days, don't know their origins. That's the Turing machine! It's what makes the whole computer
    work." You try to explain that Turing machines are merely models of computation, but he cuts you off. "No, see,
    that's just what they want you to think. Ultimately, inside every CPU, there's a Turing machine driving the whole
    thing! Too bad this one's broken. We're doomed!"

    You ask how you can help. "Well, unfortunately, the only way to get the computer running again would be to create
    a whole new Turing machine from scratch, but there's no way you can-" He notices the look on your face, gives you
    a curious glance, shrugs, and goes back to sweeping the floor.
    ...
    pause; once it does, count the number of times 1 appears on the tape. In the above example, the diagnostic
    checksum is 3.

    Recreate the Turing machine and save the computer! What is the diagnostic checksum it produces once it's
    working again?

    --- Part Two ---
    The Turing machine, and soon the entire computer, springs back to life. A console glows dimly nearby,
...
START = 'A'
CHECK = 12_425_180
RAW_STATE_MAP = [r for i, r in enumerate(INPUT.split('\n\n')) if i > 0]


class Turing:
    def __init__(self, start, raw_state_map):
        self.steps = 0
        self.state = start
        ...
        for block in raw_state_map:
            raw_state, when_0, when_1 = block.split('\n  If the current value is ')
            state = raw_state[9]
            for raw_block in [when_0, when_1]:
                block = raw_block.split('\n')
                current_value = 1 if block[0].count('1:') == 1 else 0
                write = int(block[1][:-1].split('Write the value ')[1])
                move = 1 if block[2].count('right') else -1
                next_state = block[3][:-1].split('Continue with state ')[1]
                self.state_map[(state, current_value)] = Transition(write, move, next_state)

    def tick(self):
        current_value = self.tape[self.pointer]
        transaction = self.state_map[(self.state, current_value)]
        self.steps += 1
        self.tape[self.pointer] = transaction.write
        self.pointer += transaction.move
        self.state = transaction.next_state


def test_turing():
    turing = Turing(START, RAW_STATE_MAP)
    for _ in range(CHECK):
        turing.tick()
    assert sum(turing.tape.values()) == 3099