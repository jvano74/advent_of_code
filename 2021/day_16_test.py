...
SAMPLES = [('D2FE28', '110100101111111000101000'),
...
class Bits:
    ...
    def parse(bit_stream):
        ...
        if length_type_id == 0:
            ...
            sub_packets, rest = Bits.parse_packet_length(bit_stream[22:], sub_packet_bit_length)
            return version, type_id, sub_packets, rest  # should not have to return remaining stream?

        if length_type_id == 1:
            sub_packet_count = int(bit_stream[7:7 + 11], 2)
            sub_packets, rest = Bits.parse_packet_count(bit_stream[18:], sub_packet_count)
            return version, type_id, sub_packets, rest  # should not have to return remaining stream?
    ...
    def evaluate(stream_nest):
       ...
       Following always have exactly two sub-packets
        ...
        if type_id == 4:
            return sub_packets
        if type_id == 5:
            return 1 if Bits.evaluate(sub_packets[0]) > Bits.evaluate(sub_packets[1]) else 0
        if type_id == 6:
            return 1 if Bits.evaluate(sub_packets[0]) < Bits.evaluate(sub_packets[1]) else 0
        if type_id == 7:
            return 1 if Bits.evaluate(sub_packets[0]) == Bits.evaluate(sub_packets[1]) else 0
...
def test_bits_parse():
    ...
    assert Bits.parse(SAMPLES[2][1]) == (7, 3, [(2, 4, 1), (4, 4, 2), (1, 4, 3), (None, None, None)], '')
