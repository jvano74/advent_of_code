...
def find_base(formulary, desired_fuel=1, display_output=False):
    ...
    while len(to_make) > 0:
        for element in list(to_make):
            ...
            if display_output:
                print(
                    f"Making {number * times} of {element} to get {number_needed} using {times} recipiet of",
                    components,
                )
...
def test_submission():
    print()
    with open(Path(__file__).parent / "2019_14_input.txt") as fp:
        submission = fp.read()
...
def test_submission2():
    ...
    print(
        "Starting with formula that takes",
        ONE_FULE,
        "for one to produce",
        estimated_fule_out,
        "FUEL",
    ...
    while True:
        ...
        if needed_ore < MAX_ORE:
            ...
            print(
                "Still have",
                MAX_ORE - needed_ore,
                "ORE left which should allow for",
                delta,
                "more FUEL",
            )
            if delta == 0:
