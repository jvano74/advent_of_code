class Puzzle:
    ...
    and an imaginary "safe" tile. Let's call these three tiles from the previous row the left, center, and right
    ...
    (the first ., which means "safe"), and right (the second ., also "safe") tiles on the previous row. Because
    all of the trap rules require a trap in at least one of the previous three tiles, the first tile on this new
    row is also safe, ..

    The second character on the next row considers its left (.), center (.), and right (^) tiles from the previous
    row. This matches the fourth rule: only the right tile is a trap. Therefore, the next tile in this new row
    is a trap, ^.
    ...
    The last two characters in this new row match the first and third rules, respectively, and so they are both
    also traps, ^.

    After these steps, we now know the next row of tiles in the room: .^^^^. Then, we continue on to the next row,
    using the same rules, and get ^^..^. After determining two new rows, our map looks like this:
...
def get_next_row(current_row):
    with_walls = f'#{current_row}#'
    next_row = []
    for i in range(1, len(with_walls)-1):
        L = with_walls[i-1]
        C = with_walls[i]
        R = with_walls[i+1]
        if L == '^' and C == '^' and R != '^':
...
def test_puzzle():
    # again with this you could streamline calculation identifying when you have looped (and then identifying
    # how many additional loops you would have) but brute force works fast enough here.
    current = INPUT
    count = 0
    for _ in range(400000):
        count += current.count('.')
        current = get_next_row(current)
    assert count == 19984714
