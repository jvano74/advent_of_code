...
with open(Path(__file__).parent / "2024_01_input.txt") as fp:
    RAW_INPUT = fp.read()

SAMPLE = ["3   4", "4   3", "2   5", "1   3", "3   9", "3   3"]


def find_distance(list_of_pairs: List) -> int:
    A, B = [], []
    for raw_pair in list_of_pairs:
        str_a, str_b = raw_pair.split("   ")
        A.append(int(str_a))
        B.append(int(str_b))
    distance = 0
    A.sort()
...
def test_find_distance():
    assert find_distance(SAMPLE) == 11
    assert find_distance(RAW_INPUT.split("\n")) == 2378066


def find_similarity(list_of_pairs: List) -> int:
    A, B = [], defaultdict(int)
    for raw_pair in list_of_pairs:
        str_a, str_b = raw_pair.split("   ")
        A.append(int(str_a))
...
def test_find_similarity():
    assert find_similarity(SAMPLE) == 31
    assert find_similarity(RAW_INPUT.split("\n")) == 18934359
