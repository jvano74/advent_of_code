...
def test_submission():
    max_time = 2503
    reindeer = {re.split(' ', ln)[0]: Reindeer(*re.findall(r'\d+', ln)) for ln in open('input_day_14.txt', 'r')}
...
class Race:
    ...
    def __init__(self):
        self.max_time = 2503
        self.reindeer = {re.split(' ', ln)[0]: Reindeer(*re.findall(r'\d+', ln)) for ln in open('input_day_14.txt', 'r')}
