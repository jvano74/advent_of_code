class Puzzle:
    ...
    won by the lowest-numbered memory bank) and redistributes those blocks among the banks. To do this, it removes
    all of the blocks from the selected bank, then moves to the next (by index) memory bank and inserts one of the
    blocks. It continues doing this until it runs out of blocks; if it reaches the last memory bank, it wraps around
    to the first one.
...
def count_cycle(banks):
    previous_state = set()
    cycle_count = 0
    while '-'.join([str(b) for b in banks]) not in previous_state:
        cycle_count += 1
        previous_state.add('-'.join([str(b) for b in banks]))
        max_load = max(banks)
        pos = banks.index(max_load)
        banks[pos] = 0
...
def test_count_cycle():
    assert count_cycle(SAMPLE) == (5, [2, 4, 1, 2])
    count, repeat = count_cycle(INPUT)
    assert count == 3156
    count2, _ = count_cycle([int(s) for s in repeat])
    assert count2 == 1610
