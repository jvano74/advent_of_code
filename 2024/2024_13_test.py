...
class Puzzle:
    ...
    win as many prizes as possible? You assemble a list of every machine's
...
with open(Path(__file__).parent / "2024_13_input.txt") as fp:
    MY_ARCADE = fp.read().split("\n\n")


def calculate_claw(description: str, shifted=False):
    button_regex = r"Button [AB]: X\+(\d*), Y\+(\d*)"
    prize_regex = r"Prize: X=(\d*), Y=(\d*)"
    (a, c), (b, d) = list(re.findall(button_regex, description))
    a, b, c, d = int(a), int(b), int(c), int(d)
    prize_loc = list(
        (int(x), int(y)) for (x, y) in re.findall(prize_regex, description)
    )
    x, y = prize_loc[0]
    if shifted:
        x += 10000000000000
    ...
    det_x_A = d * x - b * y
    det_x_B = a * y - c * x
    if det_x_A % det or det_x_B % det:
        return 0
    return (3 * det_x_A + det_x_B) // det


def test_calculate_claw():
    assert sum(calculate_claw(claw) for claw in SAMPLE_ARCADE) == 480
    assert sum(calculate_claw(claw) for claw in MY_ARCADE) == 33209


def test_shifted_claw():
    assert (
        sum(calculate_claw(claw, shifted=True) for claw in SAMPLE_ARCADE)
        == 875318608908
    )
    assert (
        sum(calculate_claw(claw, shifted=True) for claw in MY_ARCADE) == 83102355665474
    )
