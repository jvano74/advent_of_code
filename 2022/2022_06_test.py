...
with open(Path(__file__).parent / "2022_06_input.txt") as fp:
    MY_INPUT = fp.read()

SAMPLE = "mjqjpqmgbljsphdztnvjfqwrcgsmlb"


def find_marker(buffer, max_len=4):
    last_4 = deque(maxlen=max_len)
    for pos, char in enumerate(buffer):
        if len(set(last_4)) == max_len:
            return pos, last_4
        last_4.append(char)


def test_find_marker():
    assert find_marker(SAMPLE)[0] == 7
    assert find_marker(MY_INPUT)[0] == 1779
    assert find_marker(SAMPLE, max_len=14)[0] == 19
    assert find_marker(MY_INPUT, max_len=14)[0] == 2635
