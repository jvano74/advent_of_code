...
def test_parse_to_day_format():
    ...
    assert parse_to_day_format(SAMPLE) == [{10: [range(5, 25), range(30, 55)]},
                                           {99: [range(40,50)]},
                                           {10: [range(24, 29)]},
                                           {99: [range(36, 46)]},
                                           {99: [range(45, 55)]}], '{0} != stuff'.format(parse_to_day_format(SAMPLE))
...
def stragety_2(raw_schedule):
    ...
    print(max_sleep)
    print(sleepiest_guard, data[sleepiest_guard].most_sleepy(), data[sleepiest_guard].sleep)
    return sleepiest_guard * data[sleepiest_guard].most_sleepy()
