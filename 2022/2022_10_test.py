...
SMALL_SAMPLE = ["noop", "addx 3", "addx -5"]

with open(Path(__file__).parent / "2022_10_large_sample.txt") as fp:
    LARGE_SAMPLE = [line.strip() for line in fp]

with open(Path(__file__).parent / "2022_10_input.txt") as fp:
...
class CPU:
    ...
    def finish_cycle(self):
        if abs(self.cycle % 40 - self.registers["X"]) <= 1:
            self.screen += "#"
        else:
            self.screen += "."
        if self.cycle % 40 == 39:
        ...
        self.cycle += 1
        if self.cycle % 40 == 20:
            self.interesting.append(self.signal_strength())
    ...
    def run(self):
        for inst in self.program:
            ...
            if argv[0] == "noop":
                self.noop()
            if argv[0] == "addx":
                self.addx(int(argv[1]))


def test_cpu():
    small = CPU(SMALL_SAMPLE)
    small.run()
    assert small.registers == {"X": -1}

    large = CPU(LARGE_SAMPLE)
    large.run()
    assert sum(large.interesting) == 13140
    assert large.screen[:-1] == "\n".join(
        [
            "##..##..##..##..##..##..##..##..##..##..",
            "###...###...###...###...###...###...###.",
            "####....####....####....####....####....",
    ...
    my_input = CPU(INPUT)
    my_input.run()
    assert sum(my_input.interesting) == 14060
    assert my_input.screen[:-1] == "\n".join(
        [
            "###...##..###..#..#.####.#..#.####...##.",
            "#..#.#..#.#..#.#.#..#....#.#..#.......#.",
            "#..#.#..#.#..#.##...###..##...###.....#.",
