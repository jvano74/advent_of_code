...
class Puzzle:
    ...
    The coordinates in this example form the following pattern, where # is a dot on the paper
    ...
    In this example, the first fold instruction is fold along y=7, which designates the line formed by
    ...
    the fold is complete, but dots will never appear exactly on a fold line. The result of doing this fold
    looks like this:
...
with open(Path(__file__).parent / "2021_13_input.txt") as fp:
    RAW_INPUT = fp.read()
...
class Paper:
    def __init__(self, raw_data):
        ...
        self.folded_grid = defaultdict(int)
        self.raw_folds = []
        self.x_folds = []
        self.y_folds = []
        raw_dots, raw_folds = raw_data.split("\n\n")
        for cx, cy in [ln.strip().split(",") for ln in raw_dots.split("\n")]:
            self.grid[Pt(int(cx), int(cy))] += 1
        for ln in raw_folds.split("\n"):
            self.raw_folds.append(ln)
            direction, val = ln.split("=")
            if direction == "fold along x":
                self.x_folds.append(int(val))
            elif direction == "fold along y":
                self.y_folds.append(int(val))

    @staticmethod
    def folded_function(pt, x_folds, y_folds):
        x, y = pt.x, pt.y
        for f in x_folds:
            x = f - abs(f - x)
        for f in y_folds:
    ...
    def make_folds(self):
        ...
        for pt in self.grid:
            new_pt = Paper.folded_function(pt, self.x_folds, self.y_folds)
            self.folded_grid[new_pt] += 1
            x_min, x_max = min(new_pt.x, x_min), max(new_pt.x, x_max)
            y_min, y_max = min(new_pt.y, y_min), max(new_pt.y, y_max)
        result = []
        for y in range(y_min, y_max + 1):
            line = []
            for x in range(x_min, x_max + 1):
                line.append("#" if Pt(x, y) in self.folded_grid else " ")
            result.append("".join(line))
        return result


def test_paper():
    sample = Paper(RAW_SAMPLE)
    sample.x_folds = []
    look = sample.make_folds()
    assert "".join(look).count("#") == 17
    sample = Paper(RAW_SAMPLE)
    look = sample.make_folds()
    print("\nSAMPLE:\n")
    print("\n".join(look))
    ...
    manual = Paper(RAW_INPUT)
    manual.y_folds = []
    manual.x_folds = manual.x_folds[0:1]
    assert "".join(manual.make_folds()).count("#") == 693
    # final input
    manual = Paper(RAW_INPUT)
    look = manual.make_folds()
    print("\nSOLUTION:\n")
    print("\n".join(look))
    print("\n\n")
    # Examining the output, the code is UCLZRAZU
