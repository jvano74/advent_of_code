...
def find_min_max_marbles(list_of_draws):
    total = defaultdict(int)
    for draw in list_of_draws:
        for color, count in draw.items():
            total[color] = max(count, total[color])
    return dict(total)


def set_power(marbles):
    power = 1
...
def under_max(test_set, max_set):
    for color, count in test_set.items():
        if color not in max_set or max_set[color] < count:
            return False
    return True


def find_valid_games(game_list, max_set=MAX_MARBLES):
    valid_games = set()
    for game, marble_sets in game_list:
        valid_game = True
        for marble_set in marble_sets:
            if not under_max(marble_set, max_set):
                valid_game = False
        if valid_game:
...
def find_min_power_sum(game_list):
    powers_sum = 0
    for _, marble_sets in game_list:
        powers_sum += set_power(find_min_max_marbles(marble_sets))
    return powers_sum
...
def test_parse_input():
    ...
    my_games = parse_input(RAW_INPUT)
    assert sum(find_valid_games(my_games)) == 2256
    print(find_min_power_sum(my_games))
