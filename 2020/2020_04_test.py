...
with open(Path(__file__).parent / "2020_04_input.txt") as f:
...
def test_part_2():
    ...
    assert (
        sum(
            [
                EnhancedPassport.from_dict(passport).is_valid()
                for passport in invalid_as_dict
            ]
        )
        == 0
    )
    ...
    assert sum(
        [EnhancedPassport.from_dict(passport).is_valid() for passport in valid_as_dict]
    ) == len(valid_as_dict)
    assert (
        sum(
            [
                EnhancedPassport.from_dict(passport).is_valid()
                for passport in parse_input(INPUTS)
            ]
        )
        == 179
    )
