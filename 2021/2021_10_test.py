...
class Puzzle:
    ...
    The navigation subsystem syntax is made of several lines containing chunks. There are one
    or more chunks on each line, and chunks contain zero or more other chunks. Adjacent chunks are
    ...
    In the above example, an illegal ) was found twice (2*3 = 6 points), an illegal ] was found
    once (57 points), an illegal } was found once (1197 points), and an illegal > was found once
...
with open(Path(__file__).parent / "2021_10_input.txt") as fp:
...
def completion_score(remaining):
    score = 0
    char_value = {"(": 1, "[": 2, "{": 3, "<": 4}
    while len(remaining):
        score *= 5
        score += char_value[remaining.pop()]
    return score
