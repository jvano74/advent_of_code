class Puzzle:
    ...
    To get past the door, you first swipe a keycard (no problem; there was one on a nearby desk). Then, it displays
    a code on a little screen, and you type that code on a keypad. Then, presumably, the door unlocks.
    ...
    are your puzzle input. The screen is 50 pixels wide and 6 pixels tall, all of which start off, and is capable
...
class TinyScreen:
    ...
    def update(self, instruction):
        ...
        if bits[0] == 'rect':
            ...
            dist = int(bits[4])
            if bits[1] == 'row':
                shifted = [self.display[base][(x - dist) % self.x_max] for x in range(self.x_max)]
                for x in range(self.x_max):
                    self.display[base][x] = shifted[x]
            if bits[1] == 'column':
                shifted = [self.display[(y - dist) % self.y_max][base] for y in range(self.y_max)]
