class Puzzle:
    ...
    such as xyyx or abba. However, the IP also must not have an ABBA within any hypernet sequences, which
    ...
    (outside any square bracketed sections), and a corresponding Byte Allocation Block, or BAB, anywhere in
    the hypernet sequences. An ABA is any three-character sequence which consists of the same character twice
    with a different character between them, such as xyx or aba. A corresponding BAB is the same characters
...
def supports_ssl(ip):
    ...
    for aba in possible_aba:
        ...
        if bab in possible_bab:
            ...
            #m = possible_bab[bab]
            #if n < m:
            #    print(f'{aba},{bab} : {ip[:n]}-->{ip[n:n+3]}<--{ip[n+3:m]}==>{ip[m:m+3]}<=={ip[m+3:]}')
            #if m < n:
            #    print(f'{aba},{bab} : {ip[:m]}==>{ip[m:m+3]}<=={ip[m+3:n]}-->{ip[n:n+3]}<--{ip[n+3:]}')
            return True
    return False
