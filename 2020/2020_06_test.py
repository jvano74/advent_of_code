...
with open(Path(__file__).parent / "2020_06_input.txt") as f:
    RAW = re.split("\n\n", f.read())
    INPUT = [[p.strip() for p in re.split("\n", g)] for g in RAW]


def test_INPUT():
    assert len(INPUT) == 459
    assert len(INPUT[0]) == 4
...
def count_group2(group):
    yes = defaultdict(int)
    for person in group:
        for question in person:
            yes[question] += 1
    group_size = len(group)
    return sum(cnt == group_size for question, cnt in yes.items())


def test_count_group():
    assert sum(count_group(g) for g in SAMPLE) == 11
    assert sum(count_group(g) for g in INPUT) == 6161


def test_count_group2():
    assert sum(count_group2(g) for g in SAMPLE) == 6
    assert sum(count_group2(g) for g in INPUT) == 2971
