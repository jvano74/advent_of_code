...
def list_to_frequency_list(msgs):
    ...
    for msg in msgs:
        for i, c in enumerate(msg):
            result[i][c] += 1
    #return ''.join([sorted(freq_dict.items(), key=lambda x, v: -v)[0] for freq_dict in result])
    return ''.join([sorted(freq_dict.items(), key=lambda kv: -kv[1])[0][0] for freq_dict in result])
