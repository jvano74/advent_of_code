...
def day02():
    ...
    the prototype fabric, the design plans, everything! Nobody on the team can even seem to remember important details
    of the project!"

    "Wouldn't they have had enough fabric to fill several boxes in the warehouse? They'd be stored together, so
    the box IDs should be similar. Too bad it would take forever to search the warehouse for two similar box IDs..."
...
with open(Path(__file__).parent / "2018_02_input.txt") as f:
...
def checksum(word_list):
    return sum([has_double(w) for w in word_list]) * sum(
        [has_triple(w) for w in word_list]
    )


def off_by_one_character(word1, word2):
    errors = 0
    match = ""
...
def find_similar(word_list):
    for i, w1 in enumerate(word_list):
        for w2 in word_list[i + 1 :]:
            if off_by_one_character(w1, w2):
                return off_by_one_character(w1, w2)[1]
...
def test_part1():
    assert [has_double(s[0]) for s in SAMPLE] == [s[1] for s in SAMPLE]
    assert [has_triple(s[0]) for s in SAMPLE] == [s[2] for s in SAMPLE]
    assert checksum([s[0] for s in SAMPLE]) == 12
    assert checksum(INPUTS) == 7221
