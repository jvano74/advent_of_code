...
def find_fuel(mass):
    ...
    To accurately calculate his position in space, safely align his warp drive, and return to Earth
    in time to save Christmas, he needs you to bring him measurements from fifty stars.
...
def test_find_total_fuel():
    assert find_total_fuel(14) == 2
    assert find_total_fuel(1969) == 966
...
def test_submission():
    assert (
        sum(
            [
                find_fuel(int(m))
                for m in open(Path(__file__).parent / "2019_01_input.txt")
            ]
        )
        == 3263320
    )
