...
def mine(key,depth=5):
    ...
    the MD5 hash is some secret key (your puzzle input, given below) followed by a number in decimal. To mine AdventCoins,
    ...
    while True:
        ...
        if hash[:depth] == '0'*depth:
            return (ans,hash)
        ans += 1

def test_mine():
    assert mine('abcdef')[0] == 609043
    assert mine('pqrstuv')[0] == 1048970
    assert mine('iwrupvqb')[0] == 346386
    assert mine('iwrupvqb',depth=6)[0] == 9958218
