...
with open(Path(__file__).parent / "2019_19_input.txt") as fp:
    raw = fp.read()
...
class Image:
    ...
    def does_region_fit_in_beam(self, box: BoxRange):
        ...
        p4 = box.lr
        if (
            self.is_point_in_beam(p1.x, p1.y)
            and self.is_point_in_beam(p4.x, p1.y)
            and self.is_point_in_beam(p1.x, p4.y)
            and self.is_point_in_beam(p4.x, p4.y)
        ):
            return True
        return False
