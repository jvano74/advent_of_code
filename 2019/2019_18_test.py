        ...
        for k1 in all_positions:
            for k2 in all_positions:
                if k2 == k1:
                    continue
                try:
                    path, keys = self.shortest_path(
                        all_positions[k1], all_positions[k2]
                    )
...
with open(Path(__file__).parent / "2019_18_input.txt") as fp:
    SUBMISSION = fp.read()


with open(Path(__file__).parent / "2019_18_input_pt2.txt") as fp:
    SUBMISSION2 = fp.read()
...
def test_create_maze():
    ...
    assert maze1.key_to_key_distances() == {
        ("@0", "a"): (set(), 3),
        ("a", "@0"): (set(), 3),
        ("@0", "b"): ({"a"}, 5),
        ("b", "@0"): ({"a"}, 5),
