...
with open(Path(__file__).parent / "2021_01_input.txt") as fp:
...
def count_increase(depths):
    return sum(
        [1 if depths[i] > depths[i - 1] else 0 for i, d in enumerate(depths) if i > 0]
    )


def count_window_increase(depths):
    return sum(
        [1 if depths[i] > depths[i - 3] else 0 for i, d in enumerate(depths) if i > 2]
    )


def test_count_increase():
    assert count_increase(SAMPLE) == 7
    assert count_increase(INPUTS) == 1502


def test_window_count_increase():
    assert count_window_increase(SAMPLE) == 5
    assert count_window_increase(INPUTS) == 1538
