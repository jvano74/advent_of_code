...
class Puzzle:
    ...
    universe expanded in the time it took the light from those galaxies to reach
    the observatory.
    ...
    Your puzzle answer was 458191688761.
...
class Galaxy:
    def __init__(self, raw_map) -> None:
        self.map = set()
        self.clear_columns = [1] * len(raw_map[0])
        self.clear_rows = []
        for y, raw_line in enumerate(raw_map):
            self.clear_rows.append(1 if raw_line.replace(".", "") == "" else 0)
            for x, c in enumerate(raw_line):
                if c == "#":
                    self.clear_columns[x] = 0
                    self.map.add((x, y))

    def dist(self, a, b, expansion_factor=2):
        x_min = min(a[0], b[0])
        x_max = max(a[0], b[0])
        x_extra = sum(self.clear_columns[i] for i in range(x_min, x_max + 1))
        y_min = min(a[1], b[1])
        y_max = max(a[1], b[1])
        y_extra = sum(self.clear_rows[i] for i in range(y_min, y_max + 1))
        return (
            (x_max - x_min)
            + (y_max - y_min)
            + (x_extra + y_extra) * (expansion_factor - 1)
        )

    def total_distance(self, expansion_factor=2):
        total = 0
        for a, b in combinations(self.map, 2):
            total += self.dist(a, b, expansion_factor=expansion_factor)
        return total
...
def test_galaxy():
    test_galaxy = Galaxy(RAW_GALAXY)
    assert test_galaxy.total_distance() == 374
    assert test_galaxy.total_distance(expansion_factor=10) == 1030
    assert test_galaxy.total_distance(expansion_factor=100) == 8410

    my_galaxy = Galaxy(RAW_INPUT)
    assert my_galaxy.total_distance() == 9605127
    assert my_galaxy.total_distance(expansion_factor=1000000) == 458191688761
