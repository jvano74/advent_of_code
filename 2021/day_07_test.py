...
def min_moves(depths: List) -> int:
    ...
    return sum(target_level - cl if target_level > cl else cl - target_level for cl in depths)
...
def expensive_move_cost(depths: List, target_level: int) -> int:
    return sum(((target_level - cl) * (target_level - cl + 1)) // 2 if target_level > cl
               else ((cl - target_level) * (cl - target_level + 1)) // 2 for cl in depths)
