...
SAMPLE_FAC = {"e": 1, "hy_c": 1, "li_c": 1, "hy_g": 2, "li_g": 3}
    ...
    for component, floor in fac_map.items():
        if component != "e":
            ...
            if c_type == "c" and fac_map[c_name + "_g"] != floor:
                c_on_floor = set(
                    c for c, f in fac_map.items() if f == floor and c != "e"
                )
                for c in c_on_floor:
                    if c.split("_")[1] == "g":
                        return False
...
def fac_open_moves(fac_map):
    ...
    c_on_elevator_floor = list(
        c for c, f in fac_map.items() if f == elevator_floor and c != "e"
    )
    floors = []
    if elevator_floor == 1:
        floors = [2]
    ...
    while len(exploring) > 0:
        ...
        if fac_is_complete(fac_to_test):
            ...
            for mv in moves:
                ...
                possible_fac_hash = "".join(
                    sorted(f"{i}:{f}" for i, f in possible_fac.items())
                )
                if fac_is_safe(possible_fac):
                    if possible_fac_hash in history and history[possible_fac_hash] <= (
                        move_count + 1
                    ):
                        pass
                    else:
