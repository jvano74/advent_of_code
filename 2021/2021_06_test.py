...
class Puzzle:
    ...
    can model each fish as a single number that represents the number of days until it creates a
    new lanternfish.
    ...
    Realizing what you're trying to do, the submarine automatically produces a list of the
    ages of several hundred nearby lanternfish (your puzzle input). For example, suppose
...
with open(Path(__file__).parent / "2021_06_input.txt") as fp:
    INPUTS = [int(n) for n in fp.read().strip().split(",")]


def count_fish(initial_numbers, final_day):
    starting_array = 9 * [0]
    for d in initial_numbers:
        starting_array[d] += 1
    delays = deque(starting_array)
    for day_num in range(final_day):
        delays.rotate(-1)
        delays[6] += delays[8]
    return sum(delays)
...
def test_count_fish():
    assert count_fish(SAMPLE, 80) == 5934
    assert count_fish(INPUTS, 80) == 350149
    assert count_fish(SAMPLE, 256) == 26984457539
    assert count_fish(INPUTS, 256) == 1590327954513
