...
with open(Path(__file__).parent / "2020_03_input.txt") as f:
    INPUTS = [line.strip() for line in f]


def count_collisions(grid, run=3, fall=1):
    hits = 0
    pos = 0
...
def collision_product(grid):
    damage = 1
    slopes = [(1, 1), (3, 1), (5, 1), (7, 1), (1, 2)]
    for run, fall in slopes:
        damage *= count_collisions(grid, run, fall)
    return damage


def test_count_collisions():
    assert count_collisions(SAMPLE) == 7
    assert count_collisions(INPUTS) == 171
    assert collision_product(SAMPLE) == 336
    assert collision_product(INPUTS) == 1206576000
