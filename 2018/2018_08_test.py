...
class Puzzle:
    ...
    The sleigh is much easier to pull than you'd expect for something its weight. Unfortunately, neither you
    nor the Elves know which way the North Pole is from here.

    You check your wrist device for anything that might help. It seems to have some kind of navigation
    system! Activating the navigation system produces more bad news: "Failed to start navigation system.
    Could not read software license file."

    The navigation system's license file consists of a list of numbers (your puzzle input). The numbers define
    a data structure which, when processed, produces some kind of tree that can be used to calculate the license
    number.
...
SAMPLE = [int(d) for d in "2 3 0 3 10 11 12 1 1 0 1 99 2 1 1 2".split(" ")]

with open(Path(__file__).parent / "2018_08_input.txt") as fp:
    INPUT = [int(line.strip()) for line in fp.read().split(" ")]
...
class Tree:
    def __init__(self, data):
        ...
        while pos < len(data):
            while len(stack) > 0 and stack[-1][1:3] == [0, 0]:
                node_number, _, _ = stack.pop()
            ...
            if header:
                ...
                if len(stack) > 0:
                    ...
                    self.nodes[top_of_stack[0]]["children"].append(node_number)
                num_children = data[pos]
                num_meta = data[pos + 1]
                self.nodes[node_number] = {"loc": pos, "children": [], "meta": []}
                stack.append([node_number, num_children, num_meta])
                pos += 2
                header = False
            else:
                top_of_stack = stack[-1]
                if top_of_stack[1] > 0:
                    top_of_stack[1] -= 1
                    header = True
                elif top_of_stack[1] == 0:
                    top_of_stack[2] -= 1
                    if "meta_start" not in self.nodes[top_of_stack[0]]:
                        self.nodes[top_of_stack[0]]["meta_start"] = pos
                        self.nodes[top_of_stack[0]]["meta_sum"] = 0
                    self.meta_sum += data[pos]
                    self.nodes[top_of_stack[0]]["meta"].append(data[pos])
                    self.nodes[top_of_stack[0]]["meta_sum"] += data[pos]
                    pos += 1
    ...
    def node_value(self, node):
        ...
        if len(children) == 0:
            return self.nodes[node]["meta_sum"]
        node_sum = 0
        for child_num in self.nodes[node]["meta"]:
            if 1 <= child_num <= len(children):
                node_sum += self.node_value(children[child_num - 1])
        return node_sum


def test_sum_of_meta_dat():
    my_sample = Tree(SAMPLE)
    assert my_sample.meta_sum == 138
    assert my_sample.node_value(0) == 0
    assert my_sample.node_value(1) == 66
    puzzle = Tree(INPUT)
    assert puzzle.meta_sum == 43825
    assert puzzle.node_value(1) == 19276
