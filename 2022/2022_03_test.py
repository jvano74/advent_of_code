...
with open(Path(__file__).parent / "2022_03_input.txt") as fp:
...
def split(pack):
    mid = len(pack) // 2
    return set(pack[:mid]), set(pack[mid:])
...
def score(a, b):
    return sum(letter_value(x) for x in a & b)


def badge_scores(elves):
    total = 0
    elves_copy = elves[:]
    while len(elves_copy) > 0:
        badge = set(elves_copy.pop()) & set(elves_copy.pop()) & set(elves_copy.pop())
        total += letter_value(badge.pop())
    return total


def test_score():
    assert [score(*split(x)) for x in SAMPLE] == [16, 38, 42, 22, 20, 19]
    # Lowercase item types a through z have priorities 1 through 26.
    # Uppercase item types A through Z have priorities 27 through 52.
...
def test_badge():
    ...
    assert badge_scores(INPUTS) == 2415
