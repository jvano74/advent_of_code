...
with open(Path(__file__).parent / "2020_09_input.txt") as fp:
    INPUT = [int(num.strip()) for num in fp]


def generate_pairs(pair_list):
    pair_sums = set()
    for i, x in enumerate(pair_list):
        for y in pair_list[i + 1 :]:
            pair_sums.add(x + y)
    return pair_sums


def find_first_fail(transmission, pre):
    for i, x in enumerate(transmission[pre:]):
        if x not in generate_pairs(transmission[i : i + pre]):
            return x


def test_find_first_fail():
    assert find_first_fail(SAMPLE, 5) == 127
    assert find_first_fail(INPUT, 25) == 1309761972


def find_range(transmission, target):
    for low_end in range(len(transmission)):
        for high_end in range(low_end + 1, len(transmission)):
            if sum(transmission[low_end:high_end]) == target:
                # return low_end, high_end
                return max(transmission[low_end:high_end]) + min(
                    transmission[low_end:high_end]
                )


def test_find_range():
    assert find_range(SAMPLE, 127) == 62
    assert find_range(INPUT, 1309761972) == 6177989832
