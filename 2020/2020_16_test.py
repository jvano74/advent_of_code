...
class Ticket:
    ...
    fields in every ticket is named class and can be any value in the ranges 1-3 or 5-7 (inclusive, such
    ...
    the ticket in the order they appear; every ticket has the same format.
    ...
    as 101,102,103,104,301,302,303,401,402,403; of course, the actual train tickets you're looking at
...
with open(Path(__file__).parent / "2020_16_input.txt") as fp:
    raw = fp.read()
    raw_rules, your_ticket, nearby_tickets = raw.split("\n\n")
    INPUT_RULES = [r.strip() for r in raw_rules.split("\n")]
    INPUT_YOUR_TICKET = [int(r) for r in your_ticket.split(":\n")[1].strip().split(",")]
    INPUT_NEARBY_TICKETS = [
        [int(r) for r in line.strip().split(",")]
        for line in nearby_tickets.split(":\n")[1].split("\n")
    ]

SAMPLE_RULES = ["class: 1-3 or 5-7", "row: 6-11 or 33-44", "seat: 13-40 or 45-50"]

SAMPLE_YOUR_TICKET = [7, 1, 14]

SAMPLE_NEARBY_TICKETS = [[7, 3, 47], [40, 4, 50], [55, 2, 20], [38, 6, 12]]


class TicketValidator:
    def __init__(self, rules, tickets=None):
        if tickets is None:
            tickets = []
        self.fields = defaultdict(set)
        for pos, rule in enumerate(rules):
            name, rest = rule.split(": ")
            lower_range, _, upper_range = rest.split(" ")
            a, b = lower_range.split("-")
            self.fields[name].update(range(int(a), int(b) + 1))
            a, b = upper_range.split("-")
            self.fields[name].update(range(int(a), int(b) + 1))
        self.valid_for_any = set()
        for name in self.fields:
            self.valid_for_any.update(self.fields[name])
        self.valid_tickets = []
        for ticket in tickets:
            self.load_ticket(ticket)

    def sum_of_invalid(self, ticket):
        return sum(f for f in ticket if f not in self.valid_for_any)

    def load_ticket(self, ticket):
        if ticket in self.valid_tickets:
            return True
        if all((f in self.valid_for_any) for f in ticket):
            self.valid_tickets.append(ticket)
            return True
        return False

    def check_field(self, position):
        possible = set(self.fields)
        for field in self.fields:
            for ticket in self.valid_tickets:
                if ticket[position] not in self.fields[field]:
                    possible.discard(field)
        return possible

    def identify_fields(self):
        first_pass = {i: self.check_field(i) for i in range(len(self.fields))}
        found_fields = {}
        while len(first_pass) > 0:
            pos_to_del = set()
            for pos in first_pass:
                first_pass[pos] -= set(found_fields)
                if len(first_pass[pos]) == 1:
                    field_name = first_pass[pos].pop()
                    found_fields[field_name] = pos
                    pos_to_del.add(pos)
...
def test_ticket_validation():
    test_validator = TicketValidator(SAMPLE_RULES, SAMPLE_NEARBY_TICKETS)
    assert [test_validator.load_ticket(ticket) for ticket in SAMPLE_NEARBY_TICKETS] == [
        True,
        False,
        False,
        False,
    ...
    assert (
        sum(test_validator.sum_of_invalid(ticket) for ticket in SAMPLE_NEARBY_TICKETS)
        == 71
    )

    validator = TicketValidator(INPUT_RULES)
    assert (
        sum(validator.sum_of_invalid(ticket) for ticket in INPUT_NEARBY_TICKETS)
        == 26988
    )


SAMPLE_RULES2 = ["class: 0-1 or 4-19", "row: 0-5 or 8-19", "seat: 0-13 or 16-19"]

SAMPLE_YOUR_TICKET2 = [11, 12, 13]

SAMPLE_NEARBY_TICKETS2 = [[3, 9, 18], [15, 1, 5], [5, 14, 9]]


def test_check_field():
    test_validator = TicketValidator(SAMPLE_RULES2, SAMPLE_NEARBY_TICKETS2)
    assert test_validator.check_field(0) == {"row"}
    assert test_validator.check_field(1) == {"row", "class"}
    assert test_validator.check_field(2) == {"row", "class", "seat"}
    assert test_validator.identify_fields() == {"class": 1, "row": 0, "seat": 2}

    validator = TicketValidator(INPUT_RULES, INPUT_NEARBY_TICKETS)
    fields = validator.identify_fields()
    departure_product = 1
    for field in fields:
        if field[0:9] == "departure":
            departure_product *= INPUT_YOUR_TICKET[fields[field]]
    assert departure_product == 426362917709
